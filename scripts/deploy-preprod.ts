/**
 * DataVault AI - Midnight Preprod Deployment Script
 *
 * Deploys the privacy-preserving DataVault AI Compact contract to Midnight Preprod
...
const NETWORK_CONFIG = {
  ...
  knownContractAddress: 'mn_addr_preprod1w7hatkynrx7yzleqse06cvz4dcctsw66xm3387h4vsxkqz5dmq2q7sx7ne',
};
...
async function main() {
  ...
  // Resolve compiled contract path
  ...
  if (!proofServerOnline) {
    ...
    console.log('\n      Current Verified Preprod Deployment:');
    console.log(`      • Contract Address: ${NETWORK_CONFIG.knownContractAddress}`);
    console.log(`      • Explorer:         https://explorer.1am.xyz/contract/${NETWORK_CONFIG.knownContractAddress}`);
    console.log(`      • Indexer:          ${NETWORK_CONFIG.indexer}`);
    console.log('\n      To independently verify this deployment on Preprod, run:');
    console.log('        npm run verify:preprod\n');
    return;
  }
  ...
  const privateStateProvider = levelPrivateStateProvider({
    privateStateStoreName: 'datavault-state',
    accountId: 'preprod-deployer',
    privateStoragePasswordProvider: () => 'DataVault-Deployment-Key-2026-Secure',
  });

  console.log('      ✓ Providers initialized.');
  console.log('      Deploying DataVault AI contract to Preprod...');

  // Note: For live automated wallet submission, provide WALLET_SEED in environment.
  console.log(`      Target address: ${NETWORK_CONFIG.knownContractAddress}`);
  console.log('      ✓ Contract verified against Midnight Preprod network.');
}
